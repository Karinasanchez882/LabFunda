#include "iostream"
#include "string" //librerias que se utilizaron
#include  "string.h"
...
struct costoporarticulo{ //estructura que contiene los datos
  ...
  void mostrar() //funcion mostrar 
 {
  ...
  void datos() // funcion que calcula nombre,precio de los productos
  {
    ...
    cin>>p;
    for(int i=0;i<p;i++)//recorre los datos 
    {
      cout<<"ingrese cantidad comprada de cada producto en forma descendente :";
      cin>>pre[i]; //pide datos mediante un arreglo
    }
    ...
    for(int i=0; i<1; i++){ //calculo de suma de productos
    cos = pre[i] * precio[i] + costoporarticulo ; 
    cout<<"LA SUMA TOTAL ES :"<<cos+1<<" "; 
    }
...
int main()//funcion principal main 
{
    ...
    cin>>n;
    cin.ignore(100,'\n');
    costoporarticulo listado[n];//declaracion del arreglo
    for(i=0;i<n;i++)
    {
        cout<<"nombre cliente :";
        getline(cin,cad,'\n');//para uso de cadenas
